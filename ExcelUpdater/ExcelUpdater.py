...
if __name__ == "__main__":
    try:
        # Read Data
        files = os.listdir() # List all files in root directory
        updates = []
        databasen = ''
        for file in files:
        ...
        while not new_file: # Make sure the new DB is being saved to a new file to prevent errors.
            if os.path.isfile('updated_'+str(filei)+'.xlsx'):
                filei += 1
            else:
                new_file = True
        new_name = 'updated_'+str(filei)+'.xlsx'
        database.to_excel(new_name, index=False)
        print('New DataBase saved to ' + new_name)
    except Exception as e:
        print(e)
